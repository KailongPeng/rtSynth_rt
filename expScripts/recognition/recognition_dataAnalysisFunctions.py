...
def recognition_preprocess_2run(cfg,scan_asTemplate): 
    ...
    cmd=f"flirt -ref {cfg.templateFunctionalVolume} \
        -in {tmp[int(len(tmp)/2)]} \
        -out {cfg.templateFunctionalVolume_converted} -dof 6" # flirt -ref $TEMPLATE_bet -in $ANAT_bet -omat $ANAT2FUNC -out $ANATinFUNC -dof 6
    print(cmd)
    call(cmd,shell=True) 
...
def minimalClass(cfg,testRun=None,recordingTxt=None,forceGreedy="_"):
    '''
    purpose: 
    ...
    if forceGreedy=="forceGreedy": # 当使用forceGreedy的时候，只使用当前的session的数据进行模型的训练和
        print("running forceGreedy")
        actualRuns_preDay = []
        actualRuns_prepreDay = []
    elif forceGreedy=="compare_forceGreedy": # 当使用compare_forceGreedy的时候，只使用当前的session的数据进行模型的训练和
        print("running compare_forceGreedy")
        actualRuns_preDay = []
        actualRuns_prepreDay = []
    elif forceGreedy=="_":
        print("running normal: 8 runs")
        if len(actualRuns) < 8:
